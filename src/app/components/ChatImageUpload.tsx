  ...
  const handleFileSelect = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file && file.type.startsWith('image/')) {
      if (file.size > 5 * 1024 * 1024) {
        alert(feedbackCopy.imageMax5mb)
        return
      }
    ...
    <div className="relative">
      ...
      {preview ? (
        ...
        <button
          onClick={handleClick}
          disabled={disabled}
          className="w-10 h-10 bg-amber-100 hover:bg-amber-200 rounded-lg flex items-center justify-center text-amber-700 transition disabled:opacity-50 disabled:cursor-not-allowed"
        >
