...
import TiendaBuyerBar from '@/app/components/tienda/TiendaBuyerBar'
import TiendaOwnerPanel from '@/app/components/tienda/TiendaOwnerPanel'
import QuickPublishModal from '@/app/components/tienda/QuickPublishModal'
import AddProductModal from '@/app/components/tienda/AddProductModal'
import EditProductModal from '@/app/components/tienda/EditProductModal'
import { formatPrice, buildPublicProductUrl, INVENTARIO_API } from '@/lib/tienda/productUtils'
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      {!isOwner && canUseCart && (
        <TiendaBuyerBar
          cartCount={cartCount}
          onScrollCatalog={() => document.getElementById('tienda-catalogo')?.scrollIntoView({ behavior: 'smooth' })}
      ...
      <div id="tienda-catalogo" className="max-w-5xl mx-auto px-4 py-6 space-y-3">
        ...
        {isOwner && (
          <TiendaOwnerPanel
            ...
            ownerToolsOpen={ownerToolsOpen}
            onToggleOwnerTools={() => setOwnerToolsOpen((o) => !o)}
            onQuickPublish={() => setShowQuickPublish(true)}
            onAdvancedMode={() => setShowAddModal(true)}
            showAddStoreCategory={showAddStoreCategory}
            onShowAddStoreCategory={setShowAddStoreCategory}
            newStoreCategory={newStoreCategory}
            onNewStoreCategoryChange={setNewStoreCategory}
            savingStoreCategory={savingStoreCategory}
            onAddStoreCategory={handleAddStoreCategory}
            checkoutMode={checkoutMode}
            onCheckoutModeChange={setCheckoutMode}
            tab={tab}
            onTabCatalogo={() => setTab('catalogo')}
            onTabStats={() => { setTab('stats'); fetchStats() }}
          />
        )}
      </div>
