...
const STATUS_MAP = {
  active: { label: 'Disponible ahora', color: 'bg-teal-100 text-teal-800', dot: 'bg-teal-500' },
  intermediate: { label: 'Disponibilidad flexible', color: 'bg-amber-100 text-amber-900', dot: 'bg-amber-500' },
...
export default function WorkerPublicProfile() {
  ...
  return (
    <div className="max-w-md mx-auto bg-gray-50 min-h-screen relative shadow-2xl overflow-hidden font-sans pb-24 text-gray-800">
      ...
      {/* Banner */}
      <div className="bg-gradient-to-r from-teal-600 via-teal-700 to-teal-900 h-36 relative rounded-b-[2rem]">
        ...
        <div className="absolute -bottom-14 left-1/2 transform -translate-x-1/2">
          <div className="relative">
            {avatar
              ...
              : <div className="w-28 h-28 rounded-full border-4 border-white shadow-lg bg-gradient-to-br from-teal-500 to-teal-800 flex items-center justify-center text-3xl font-black text-white">{initials}</div>
            }
            {isActive && <span className="absolute bottom-2 right-2 w-5 h-5 bg-teal-500 border-2 border-white rounded-full animate-pulse" />}
          </div>
        </div>
      </div>
      ...
      <div className="pt-16 px-6 text-center">
        ...
        <div className="flex justify-center gap-6 mt-4 pb-4 border-b border-gray-200">
          ...
          <div className="text-center">
            <p className="text-lg font-bold text-amber-700">🔥 {freshScore}</p>
      ...
      <div className="px-6 py-4 space-y-6">
        ...
        <div className="flex items-center justify-between bg-white p-4 rounded-2xl shadow-sm border border-gray-100">
          <div className="flex items-center gap-2">
            {isActive ? (
              ...
              <span className={`w-3 h-3 rounded-full ${isIntermediate ? 'bg-amber-400' : 'bg-gray-300'}`} />
            )}
            <span className={`text-sm font-bold ${isActive ? 'text-teal-800' : isIntermediate ? 'text-amber-800' : 'text-gray-500'}`}>
        ...
        {worker.user.is_pioneer && (
          <div className="bg-gradient-to-r from-amber-50 to-orange-50 border border-amber-200 p-3 rounded-xl flex items-center gap-3">
            <div className="text-2xl">🚀</div>
            <div>
              <p className="text-sm font-bold text-amber-950">Usuario Pionero</p>
              <p className="text-xs text-amber-900/90">De los primeros expertos en JobsHours.</p>
            </div>
          </div>
        )}
        ...
        <div className="flex gap-3 pt-4 border-t border-gray-200">
          <button onClick={() => window.open(`https://wa.me/?text=${encodeURIComponent(shareText + '\n' + shareUrl)}`, '_blank')}
            className="flex-1 flex items-center justify-center gap-2 bg-amber-50 text-amber-900 py-2.5 rounded-xl font-semibold text-sm border border-amber-200 hover:bg-amber-100 transition">
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 p-4 shadow-[0_-10px_15px_-3px_rgba(0,0,0,0.05)] max-w-md mx-auto z-50">
        <button onClick={() => router.push(`/?worker=${worker.id}`)}
          className="w-full bg-gradient-to-r from-amber-500 to-orange-600 hover:from-amber-400 hover:to-orange-500 text-white font-bold text-lg py-3.5 rounded-xl shadow-lg shadow-amber-500/25 transition-colors flex justify-center items-center gap-2">
